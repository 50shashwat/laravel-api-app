...
```
    GET : api/v1/currencies (without token)
```
...
```
    GET : api/v1/countries (without token)
```
...
```
    POST : /api/v1/verifyReceipt (without token)
```

  Name              | Type                 | Required
  -------------     | -------------        | -------------
  token             | text(base64(code))   | true
...
```
    POST : /api/v1/users/makePremium (with token)
```

  Name              | Type                 | Required
  -------------     | -------------        | -------------
  transaction_id    | text                 | true
  purchase_date     | text                 | false
  product_id        | text                 | false
  expired_at        | text                 | false
